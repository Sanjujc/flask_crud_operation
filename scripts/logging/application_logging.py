...
log_file = os.path.join(app_configurations.LOG_FILE_NAME + "_" + time.strftime("%Y%m%d") + '.logs')
...
if 'console' in LOG_HANDLERS:
    # Adding the logs Console handler to the logger
    console_handler = StreamHandler(sys.stdout)
    console_handler.setFormatter(formatter)
    logger.addHandler(console_handler)

if 'file' in LOG_HANDLERS:
    # Adding the logs file handler to the logger
